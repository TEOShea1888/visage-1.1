...
def extract_osm_features_stub(destinations_df):
    # Accept multiple possible column names
    for col in ["quality", "QualityScore", "quality_score", "overture_poi_count"]:
        if col in destinations_df.columns:
            return destinations_df[col]

    raise ValueError(
        f"Could not find a usable quality column. Available columns: {list(destinations_df.columns)}"
    )
...
def run_quality_sensitive_gravity(origins_df,
    ...
    A_j = quality_attractor(quality_scores, beta=beta).to_numpy().reshape(1, -1)
